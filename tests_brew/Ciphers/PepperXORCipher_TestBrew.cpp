...
const std::vector<unsigned char>& GetMask(std::size_t pLength) {
  ...
  if (aIt == sMasks.end()) {
    aIt = sMasks.emplace(pLength, BuildTable(pLength, 0xC8E9124Du, 0x5Cu)).first;
  }
  return aIt->second;
}
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_FlatSized(
      [](std::size_t pLength, std::size_t, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::PepperXORCipher>(aMask.data(),
                                                               aMask.size());
      },
      "PepperXORCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PepperXORCipher", aError);
}
