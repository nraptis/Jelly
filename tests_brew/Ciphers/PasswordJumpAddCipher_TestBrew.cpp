...
std::vector<unsigned char> BuildPassword(std::size_t pCaseIndex) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x801u);
...
std::vector<unsigned char> BuildJumps(std::size_t pCaseIndex, std::size_t pPasswordLength) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x802u);
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_Flat(
      ...
      "PasswordJumpAddCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PasswordJumpAddCipher", aError);
}
