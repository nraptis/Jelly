...
std::vector<unsigned char> BuildMask(std::size_t pCaseIndex) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x8D1u);
...
std::vector<unsigned char> BuildJumps(std::size_t pCaseIndex,
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x8D2u);
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_Flat(
      [](std::size_t pCaseIndex, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::CascadeJumpXORCipher>(
            aMask.data(), static_cast<int>(aMask.size()), aJumps.data(),
            static_cast<int>(aJumps.size()));
      },
      "CascadeJumpXORCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("CascadeJumpXORCipher",
                                                   aError);
}
