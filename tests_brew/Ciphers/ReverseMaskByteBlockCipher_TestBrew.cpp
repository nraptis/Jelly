...
int RunCase() {
  ...
  peanutbutter::ExecuteTestBrew_Block(
      [](int, std::size_t pCaseIndex, peanutbutter::CryptMode) {
        return std::make_unique<peanutbutter::ReverseMaskByteBlockCipher<tBlockSize>>(
            peanutbutter::test_brew::CaseMask(pCaseIndex, 0x403u));
      },
      static_cast<int>(tBlockSize), "ReverseMaskByteBlockCipher", &aError);
  return peanutbutter::test_brew::ReportBlockResult("ReverseMaskByteBlockCipher",
