...
std::vector<unsigned char> BuildPassword(std::size_t pCaseIndex) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x501u);
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_Flat(
      [](std::size_t pCaseIndex, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::PasswordAddCipher>(aPassword);
      },
      "PasswordAddCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PasswordAddCipher", aError);
}
