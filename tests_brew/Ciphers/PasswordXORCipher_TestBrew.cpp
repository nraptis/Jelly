...
std::vector<unsigned char> BuildPassword(std::size_t pCaseIndex) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x701u);
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_Flat(
      [](std::size_t pCaseIndex, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::PasswordXORCipher>(aPassword);
      },
      "PasswordXORCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PasswordXORCipher", aError);
}
