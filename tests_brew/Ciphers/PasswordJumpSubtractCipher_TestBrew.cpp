...
std::vector<unsigned char> BuildPassword(std::size_t pCaseIndex) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x811u);
...
std::vector<unsigned char> BuildJumps(std::size_t pCaseIndex, std::size_t pPasswordLength) {
  ...
  std::uint32_t aBits = peanutbutter::test_brew::CaseBits(pCaseIndex, 0x812u);
...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_Flat(
      [](std::size_t pCaseIndex, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::PasswordJumpSubtractCipher>(
            aPassword.data(), static_cast<int>(aPassword.size()), aJumps.data(),
            static_cast<int>(aJumps.size()));
      },
      "PasswordJumpSubtractCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PasswordJumpSubtractCipher",
                                                   aError);
}
