...
int main() {
  ...
  peanutbutter::ExecuteTestBrew_FlatSized(
      [](std::size_t pLength, std::size_t, peanutbutter::CryptMode)
        ...
        return std::make_unique<peanutbutter::PepperAddCipher>(aMask.data(),
                                                               aMask.size());
      },
      "PepperAddCipher", &aError);
  return peanutbutter::test_brew::ReportFlatResult("PepperAddCipher", aError);
}
