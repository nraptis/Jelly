...
bool ExecuteCase(const swift_test_data::TestCase& pCase,
  ...
  if (aBlockSizeParam == nullptr || aShiftParam == nullptr) {
  ...
  if (!swift_test_data::ReadUInt8(*aBlockSizeParam, aBlockSize) ||
      !swift_test_data::ReadInt16(*aShiftParam, aShift)) {
    std::cerr << pPath << ": bad param encoding at case " << pIndex << "\n";
    return false;
  }

  jelly::RotateBlockCipher aCipher(aBlockSize, aShift);
...
int main(int argc, char** argv) {
  return test_swift_executor::RunFiles(argc, argv, "RotateBlockCipher",
                                       ExecuteCase);
}
