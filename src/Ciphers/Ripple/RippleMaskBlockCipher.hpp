...
class RippleMaskBlockCipher final : public LayerCakeCryptDelegate {
 public:
 ...
 private:
...
#if defined(__ARM_NEON) || defined(__ARM_NEON__)
        if constexpr (kBlockSize == EB_BLOCK_SIZE_16 ||
                      kBlockSize == EB_BLOCK_SIZE_32 ||
                      kBlockSize == EB_BLOCK_SIZE_48) {
  ...
  void ApplyMaskedBlockPass(const unsigned char* pSource,
                            ...
                            CryptMode pMode) const {
    switch (mBlockSize) {
      case EB_BLOCK_SIZE_08:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_08>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      case EB_BLOCK_SIZE_12:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_12>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      case EB_BLOCK_SIZE_16:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_16>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      case EB_BLOCK_SIZE_24:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_24>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      case EB_BLOCK_SIZE_32:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_32>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      case EB_BLOCK_SIZE_48:
        ApplyMaskedBlockPassFixed<EB_BLOCK_SIZE_48>(pSource, pDestination,
                                                    pBlockCount, pPhase, pMode);
        return;
      default:
