...
class RippleBlockCipher final : public LayerCakeCryptDelegate {
 public:
  ...
  void ApplyBlockPass(const unsigned char* pSource,
    ...
    switch (mBlockSize) {
      case EB_BLOCK_SIZE_08:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_08>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      case EB_BLOCK_SIZE_12:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_12>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      case EB_BLOCK_SIZE_16:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_16>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      case EB_BLOCK_SIZE_24:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_24>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      case EB_BLOCK_SIZE_32:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_32>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      case EB_BLOCK_SIZE_48:
        ApplyBlockPassFixed<EB_BLOCK_SIZE_48>(pSource, pDestination, pBlockCount,
                                              pPhase);
        return;
      default:
