#ifndef PEANUTBUTTER_CASCADE_XOR_CIPHER_HPP_
#define PEANUTBUTTER_CASCADE_XOR_CIPHER_HPP_
...
class CascadeXORCipher final : public Crypt {
 public:
  CascadeXORCipher(const unsigned char* pMask, int pMaskLength)
 ...
 private:
  bool Apply(const unsigned char* pSource,
    ...
    if (pSource == nullptr || pDestination == nullptr || mMask == nullptr) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "null buffer in CascadeXORCipher";
      }
      return false;
    }
    ...
    if (mMaskLength <= 0 || (mMaskLength % 16) != 0) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "mask length must be a positive multiple of 16";
      }
      return false;
    }
...
#endif  // PEANUTBUTTER_CASCADE_XOR_CIPHER_HPP_
