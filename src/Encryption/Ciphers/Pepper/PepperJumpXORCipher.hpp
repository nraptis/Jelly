#ifndef PEANUTBUTTER_PEPPER_JUMP_XOR_CIPHER_HPP_
#define PEANUTBUTTER_PEPPER_JUMP_XOR_CIPHER_HPP_
...
class PepperJumpXORCipher final : public Crypt {
 public:
  PepperJumpXORCipher(const unsigned char* pMask,
                      int pMaskLength,
 ...
 private:
  bool Apply(const unsigned char* pSource,
    ...
    if (pSource == nullptr || pDestination == nullptr || mMask == nullptr ||
        mJumps == nullptr) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "null buffer in PepperJumpXORCipher";
      }
      return false;
    }
      ...
      switch (pMode) {
        case CryptMode::kNormal:
          for (std::size_t aIndex = 0; aIndex < 16; ++aIndex) {
            aDestinationPtr[aIndex] =
                static_cast<unsigned char>(aSourcePtr[aIndex] ^ aMaskPtr[aIndex]);
          }
          break;
        ...
        case CryptMode::kNeon:
          ...
          vst1q_u8(aDestinationPtr,
                   veorq_u8(vld1q_u8(aSourcePtr), vld1q_u8(aMaskPtr)));
#else
          for (std::size_t aIndex = 0; aIndex < 16; ++aIndex) {
            aDestinationPtr[aIndex] =
                static_cast<unsigned char>(aSourcePtr[aIndex] ^ aMaskPtr[aIndex]);
...
#endif  // PEANUTBUTTER_PEPPER_JUMP_XOR_CIPHER_HPP_
