#ifndef PEANUTBUTTER_PEPPER_JUMP_NOISE_XOR_CIPHER_HPP_
#define PEANUTBUTTER_PEPPER_JUMP_NOISE_XOR_CIPHER_HPP_
...
// Input shape: pLength must be a multiple of BLOCK_GRANULARITY; mask, noise, and jump lengths must be positive multiples of 16.

class PepperJumpNoiseXORCipher final : public Crypt {
 public:
  PepperJumpNoiseXORCipher(const unsigned char* pMask,
                           int pMaskLength,
 ...
 private:
  bool Apply(const unsigned char* pSource,
    ...
    if (pSource == nullptr || pDestination == nullptr || mMask == nullptr ||
      ...
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "null buffer in PepperJumpNoiseXORCipher";
      }
      return false;
    }
    ...
    if (mMaskLength <= 0 || (mMaskLength % 16) != 0 || mNoiseLength <= 0 ||
        (mNoiseLength % 16) != 0 || mJumpLength <= 0 ||
        (mJumpLength % 16) != 0) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage =
            "mask, noise, and jump lengths must be positive multiples of 16";
      }
      return false;
    }
...
#endif  // PEANUTBUTTER_PEPPER_JUMP_NOISE_XOR_CIPHER_HPP_
