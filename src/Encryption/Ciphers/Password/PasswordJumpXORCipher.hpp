#ifndef PEANUTBUTTER_PASSWORD_JUMP_XOR_CIPHER_HPP_
#define PEANUTBUTTER_PASSWORD_JUMP_XOR_CIPHER_HPP_
...
class PasswordJumpXORCipher final : public Crypt {
 public:
  PasswordJumpXORCipher(const unsigned char* pPassword,
                        int pPasswordLength,
 ...
 private:
  bool Apply(const unsigned char* pSource,
    ...
    if (pSource == nullptr || pDestination == nullptr || mPassword == nullptr ||
        mJumps == nullptr) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "null buffer in PasswordJumpXORCipher";
      }
      return false;
    }
      ...
      switch (pMode) {
        ...
        case CryptMode::kNeon:
          ...
          vst1q_u8(aDestinationPtr,
                   veorq_u8(vld1q_u8(aSourcePtr), vld1q_u8(aPasswordPtr)));
#else
          for (std::size_t aIndex = 0; aIndex < 16; ++aIndex) {
            aDestinationPtr[aIndex] = static_cast<unsigned char>(
                aSourcePtr[aIndex] ^ aPasswordPtr[aIndex]);
...
#endif  // PEANUTBUTTER_PASSWORD_JUMP_XOR_CIPHER_HPP_
